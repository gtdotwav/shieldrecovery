...
import {
  getVisibleRoutes as getVisibleRoutesAll,
  platformRoutes,
  type PlatformRoute,
  type SidebarGroup,
} from "@/components/platform/platform-routes";
...
export type { PlatformRoute, SidebarGroup };

// platformRoutes / PlatformRoute / SidebarGroup are imported above and
// re-exported for backwards compatibility. The actual route table lives in
// platform-routes.ts so it can be tree-shaken from non-shell consumers.

function getVisibleRoutes(role?: UserRole) {
  return getVisibleRoutesAll(role, {
    experimentalEnabled: appEnv.experimentalPagesEnabled,
  });
}
